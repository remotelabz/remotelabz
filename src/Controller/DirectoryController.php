...
class DirectoryController extends Controller
{
    ...
    public function moveDirectory(
        ...
        $newParent = null;
        if ($newParentId) {
            $newParent = $directoryRepo->find((int) $newParentId);
            if (!$newParent) {
    ...
    public function getDirectoryTree(
        ...
        $parent = null;
        if ($parentId) {
            $parent = $directoryRepo->find((int) $parentId);
        }
    ...
    public function batchMoveToDirectory(
        ...
        DirectoryRepository $directoryRepo,
        \App\Repository\DeviceRepository $deviceRepo,
        \App\Repository\IsoRepository $isoRepo,
        \App\Repository\OperatingSystemRepository $osRepo
    ): JsonResponse {
        $directory = $directoryRepo->find($id);
        ...
        if (!empty($data['device_ids'])) {
            foreach ($data['device_ids'] as $deviceId) {
                $device = $deviceRepo->find((int) $deviceId);
                if ($device) {
