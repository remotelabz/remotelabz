...
function checkNodeIcon($s) {
	if (preg_match('/^[A-Za-z0-9_+\\s-]*\.[png$\|.jpg$*]/', $s) && is_file(BASE_DIR.'/images/icons/'.$s)) {
		return True;
	} else {
		return False;
	}
...
function listNodeConfigTemplates() {
	$results = Array();
	foreach (scandir(BASE_DIR.'/configs') as $filename) {
		if (is_file(BASE_DIR.'/configs/'.$filename) && preg_match('/^.+\.php$/', $filename)) {
...
function listNodeIcons() {
	$results = Array();
	foreach (scandir(BASE_DIR.'/images/icons') as $filename) {
		if (is_file(BASE_DIR.'/images/icons/'.$filename) && preg_match('/^.+\.[png$\|jpg$]/', $filename)) {
